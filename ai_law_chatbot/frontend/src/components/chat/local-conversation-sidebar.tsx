    ...
    <>
      ...
      >
        <div className={cn("flex h-12 items-center border-b px-4 py-3", isCollapsed ? "justify-center" : "justify-between")}>
          {!isCollapsed && <h2 className="text-sm font-semibold truncate">Local Chats</h2>}
          <Button
            variant="ghost"
            size="sm"
        ...
        {isCollapsed && (
          <div className="flex flex-col items-center py-4 gap-4">
             <Button 
                variant="ghost" 
                size="icon" 
                onClick={() => useLocalChatStore.getState().createConversation()} 
                className="h-10 w-10 hover:bg-secondary/80 text-primary"
                title="New Chat"
             >
