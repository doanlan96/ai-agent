...
Each generated project includes a CLI tool named `ai_law_chatbot`. Run commands from the `backend/` directory:

```bash
cd backend

# Server commands
uv run ai_law_chatbot server run --reload     # Start dev server
uv run ai_law_chatbot server routes           # Show all routes
# Database commands
uv run ai_law_chatbot db migrate -m "message" # Create migration
uv run ai_law_chatbot db upgrade              # Apply migrations
uv run ai_law_chatbot db downgrade            # Rollback migration
# User commands
uv run ai_law_chatbot user create-admin       # Create admin user
uv run ai_law_chatbot user create             # Create regular user
uv run ai_law_chatbot user list               # List all users
```
...
# Server
uv run ai_law_chatbot server run --reload
uv run ai_law_chatbot server routes

# Database (Alembic wrapper)
uv run ai_law_chatbot db init
uv run ai_law_chatbot db migrate -m "Add users"
uv run ai_law_chatbot db upgrade

# Users
uv run ai_law_chatbot user create-admin       # Create admin (interactive)
uv run ai_law_chatbot user create             # Create user (interactive)
uv run ai_law_chatbot user list               # List all users
```
...
```bash
uv run ai_law_chatbot cmd seed --count 100
uv run ai_law_chatbot cmd seed --dry-run
```
...
| Start dev server | `uv run --directory backend ai_law_chatbot server run --reload` |
| Start prod server | `uv run --directory backend ai_law_chatbot server run --host 0.0.0.0 --port 8000` |
| Show routes | `uv run --directory backend ai_law_chatbot server routes` |
...
| Task | Command |
|------|---------|
| Create migration | `uv run --directory backend ai_law_chatbot db migrate -m "message"` |
| Apply migrations | `uv run --directory backend ai_law_chatbot db upgrade` |
| Rollback migration | `uv run --directory backend ai_law_chatbot db downgrade` |
| Show current | `uv run --directory backend ai_law_chatbot db current` |
| Show history | `uv run --directory backend ai_law_chatbot db history` |

### Users

| Task | Command |
|------|---------|
| Create admin | `uv run --directory backend ai_law_chatbot user create-admin` |
| Create user | `uv run --directory backend ai_law_chatbot user create` |
| List users | `uv run --directory backend ai_law_chatbot user list` |
